 ...
 * subscription (the portal verifies the payment with ITS OWN key), gets the
 * license key, saves it to config, and shows success.
 */
class Activated extends Action
{
    ...
    public function execute(): ResultInterface
    {
        ...
        if (!$sessionId) {
            ...
            return $this->resultFactory->create(ResultFactory::TYPE_REDIRECT)->setPath('canonicalhreflang/license/gate');
        }
        ...
        $block = $page->getLayout()->getBlock('canonicalhreflang.license.activated');
        if ($block) {
            $block->setData('license_key', $licenseKey)
                  ->setData('plan', $planName)
                  ->setData('error', $error)
                  ->setData('settings_url', $this->getUrl('adminhtml/system_config/edit/section/etechflow_canonical'))
                  ->setData('management_url', $this->getUrl('canonicalhreflang/license/gate'));
        }
